...
RUN apk add --no-cache ca-certificates musl-locales musl-locales-lang openssl wget && \
    wget --no-verbose \
        ...
        https://github.com/dragonwell-project/dragonwell11/releases/download/dragonwell-extended-11.0.20.17_jdk-11.0.20-ga/Alibaba_Dragonwell_Extended_11.0.20.17.8_x64_alpine-linux.tar.gz \
    && \
    mkdir -p /tmp/export &&\
    tar --extract \
